...
def get_user(username: str) -> t.Optional[str]:
    command = '/etc/anych/./checkanyu %s 1' % username
    result = os.popen(command).readlines()
    final = result[0].strip()
    return final

def cont_online(username: str) -> t.Optional[str]:
    command = '/etc/anych/./checkanyu %s 2' % username
    result = os.popen(command).readlines()
    final = result[0].strip()
    return final

def limiter_user(username: str) -> t.Optional[str]:
    command = '/etc/anych/./checkanyu %s 3' % username
    result = os.popen(command).readlines()
    final = result[0].strip()
    return final

def check_data(username: str) -> t.Optional[str]:
    command = '/etc/anych/./checkanyu %s 4' % username
    result = os.popen(command).readlines()
    final = result[0].strip()
    return final

def check_dias(username: str) -> t.Optional[str]:
    command = '/etc/anych/./checkanyu %s 5' % username
    result = os.popen(command).readlines()
    final = result[0].strip()
    return final

@app.route('/checkuserany', methods=['POST', 'GET'])
def check_user():
    if request.method == 'POST':
        ...
        try:
            return 'Cannot GET /checkuserany'
        except Exception as e:
            return jsonify({'error': str(e)})
